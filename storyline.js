...
(function () {
  ...
  function conditionColor(v) {
    // Condition mode uses the 5 discrete PMIS category colors (matching the
    // per-year cells), not a continuous RdYlGn gradient.
  ...
  const BASE_DATA_FILE = window.STORYLINE_DATA_FILE || "storyline_data_hwcounty.json";
      ...
      for (let segIdx = 0; segIdx < pts.length; segIdx++) {
        ...
        for (let i = 0; i < segPts.length; i++) {
          ...
          if (!state.paint.has(p.id) && state.colorMode === "condition" && p.yv && p.yv.length) {
            ...
            const color = painted ? paintColor(state.paint.get(p.id), p.v) : state.colorMode === "cohort" ? (IS_GEO ? shadeGray(p.v) : cohortColor(p.trackId, p.v)) : state.colorMode === "highway" ? highwayColor(p.roadbed, p.v) : state.colorMode === "pavtype" ? pavTypeColor(p.pavtype, p.v) : state.colorMode === "condition" ? conditionColor(p.v) : shadeGray(p.v);
  ...
  function appendLines(roadIdx, pts, buckets, gradientBars) {
    ...
    for (let i = 0; i < pts.length; i++) {
      ...
      if (!state.paint.has(p.id) && state.colorMode === "condition" && p.yv && p.yv.length) {
        ...
        const color = state.paint.has(p.id) ? paintColor(state.paint.get(p.id), p.v) : state.colorMode === "cohort" ? (IS_GEO ? shadeGray(p.v) : cohortColor(p.trackId, p.v)) : state.colorMode === "highway" ? highwayColor(p.roadbed, p.v) : state.colorMode === "pavtype" ? pavTypeColor(p.pavtype, p.v) : state.colorMode === "condition" ? conditionColor(p.v) : shadeGray(p.v);
  ...
  function drawHighlighted(roadIdx, segIdx) {
    ...
    for (let i = 0; i < pts.length; i++) {
      ...
      if (!state.paint.has(p.id) && BAR_ENCODING === "gradient" && state.colorMode === "condition" && p.yv && p.yv.length) {
        ...
        ctx.strokeStyle = state.paint.has(p.id) ? paintColor(state.paint.get(p.id), p.v) : state.colorMode === "cohort" ? (IS_GEO ? shadeGray(p.v) : cohortColor(p.trackId, p.v)) : state.colorMode === "highway" ? highwayColor(p.roadbed, p.v) : state.colorMode === "pavtype" ? pavTypeColor(p.pavtype, p.v) : state.colorMode === "condition" ? conditionColor(p.v) : shadeGray(p.v);
      }
      ctx.beginPath();
      ctx.moveTo(x0, p.y);
      ctx.lineTo(x1, p.y);
  ...
  // exactly as the plain-click path on non-geo pages does: out-of-range / no-hit
  // / singleton -> clearEnforce(); same cohort already enforced -> clearEnforce()
  // (toggle off); otherwise set state.enforcedAlign and recomputeEnforce().
  ...
  function onCanvasClick(evt) {
    ...
    // Shift-click enforce-aligns the clicked cohort on ANY page (including geo
    // pages, where plain click is reserved for brushing/paint). Takes precedence
    // over the paint branch and the mapMode no-op.
    if (evt.shiftKey) {
    ...
    // session's entities (overrides both the geo no-op and enforce-align).
  ...
  // entity appears in (never flips on cohort split/merge). Painted dots are
  // also colored on the geographic map (main geo pages).
  function applyPaintAt(roadIdx, segIdx, k) {
    ...
    if (armed === "__erase__") {
      ...
      // Erase can't remove individual sections from a color group cheaply, so
      // rebuild the whole map paint from the surviving state.
      syncMapPaint();
    } else {
  ...
  // Rebuild the map's persistent paint from state.paint, one paint() call per
  // distinct color (used after erase and Clear-partial operations).
  ...
  function updateColorLegend() {
    ...
    if (mode === "condition") {
      // 5 discrete PMIS categories (matching pmisCategoryColor thresholds),
      // ordered best -> worst.
      const rows = [
        [95, "Very Good"],
        [80, "Good"],
      ...
      colorLegendEl.innerHTML = rows.map(([score, label]) =>
        `<div class="legend-swatch-row"><span class="legend-swatch" style="background:${pmisCategoryColor(score)}"></span>${label}</div>`
      ).join("");
  ...
  function enforceCohortByIds(k, ids) {
    ...
    // One-shot id -> {roadIdx, segIdx} index from the loaded data (no existing
    // module-level id map to reuse; ids are otherwise resolved via road.segments).
